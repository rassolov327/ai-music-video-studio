...
// ---------- video-to-video editing (Kling 3.0 Omni Transformation) — edits a shot's EXISTING
// video by prompt (e.g. "replace the background") instead of animating a still image. Only
// offered once the shot already has a video, from either MOVIE or a video dropped straight in
// from Archive. ----------
const VIDEO_EDIT_RESOLUTIONS = ['720p', '1080p', '4k'];
...
// TASKS-tile flavor of the select (task-tile-model-select class) — mirrors
// motionControlModelSelectHtml/lipsyncModelSelectHtml in lipsync.js exactly, used when this
// draft shows up as a tile in the TASKS grid, not in the Inspector's own inline field below.
...
// Inspector slot — deliberately mirrors the Description field's own cf-field/field-group
// markup right above (shotDescInput) rather than inventing new styling, so it reads as part
// of the same panel instead of a bolted-on widget.
async function renderVideoEditSlot(scene, shot){
  ...
  freshSlot.innerHTML = `
    <div class="field-group">
      ...
      <div class="cf-field"><label>Resolution</label>
        <select id="shotVideoEditResolutionSelect">
