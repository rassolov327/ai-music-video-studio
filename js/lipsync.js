...
async function trimVideoToDuration(videoUrl, durationSec, onStatus){
  ...
  try{
    ...
    // Note: the faststart step (moving the moov metadata atom to the front, needed since
    // the file gets fetched by URL rather than uploaded directly) does NOT happen here —
    // ffmpeg.wasm's own internal mechanism for that ("second pass: moving moov atom", which
    // +faststart triggers) has a confirmed, reproducible crash regardless of file size or
    // how the command is split up. That step now happens server-side with a real ffmpeg
    // binary instead (see remuxFaststart in server.js), which doesn't have this bug.
    await ffmpeg.exec([
    ...
    if(!data || !data.buffer || data.buffer.byteLength < 1000){
      // Safety net for any ffmpeg.wasm crash mid-process — surfaces a clear error instead
      // of silently continuing with broken data that fails confusingly further downstream.
